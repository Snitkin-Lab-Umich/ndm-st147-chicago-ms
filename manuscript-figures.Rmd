...
```{r}
# Remove isolates from vSNF O and vSNF P because they didn't participate in all surveys
...
get_dups_fig1 <- function(df){
  df_sub <- df %>% 
    filter(survey %in% c(13,301,302,303)) %>% 
...
# data for figure 1
dat <- dat %>% filter(survey %in% c(13,301,302,303)) %>% filter(!get_dups_fig1(dat)) %>% 
  mutate(survey = ifelse(survey == '301', '13-1', survey),
         survey = ifelse(survey == '302', '13-2', survey),
         survey = ifelse(survey == '303', '13-3', survey))
...
`Western Asia` = "#e6beff", `Western Europe` = "#9a6324", `Chicago area`="#800000", Absent='white',"NDM_1"="dodgerblue3",'NDM_4'='lightblue','NDM_5'='springgreen','NDM_7'='skyblue',"KPC_2"='lightgrey', "KPC_3"="darkgrey",Present='black','2016'='tan1','2017'='#bdc9e1','2018'='#67a9cf','2019'='#02818a', '13'='tan1','13-1'='#bdc9e1','13-2'='#67a9cf','13-3'='#02818a',NDM="dodgerblue3", KPC="darkgrey", 'IncF plasmid'='black',' '='white',l_cols)
```
...
dat <- dat %>% 
  mutate(bla = paste0(ifelse(kpc == 'yes' & ndm != 'yes' & !is.na(ndm),'KPC',''),
         ...
         survey = ifelse(survey == '301', '13-1', survey),
         survey = ifelse(survey == '302', '13-2', survey),
         survey = ifelse(survey == '303', '13-3', survey),
         survey = as.factor(survey))

# all
summary_all <- dat %>% #group_by(factype) %>% 
  ...
  summarize(
    ...
    'Mechanical ventilation, n (%)' = paste0(
      ...
      100*round(mean(vent == 'yes' | vent == 'vent', na.rm = TRUE),2), ')'),
...
# facil_cts <- table(dat$factype[!duplicated(dat$f_id)])

summary_dat <- bind_rows(summary_facil,summary_all) %>% 
  mutate(factype = gsub('V','v',toupper(factype)),
         factype = ifelse(is.na(factype),'All',factype)) %>% 
  rename('Facility type' = factype) 

# summary_dat$`Facility type (n)` <- sapply(summary_dat$`Facility type (n)`, function(x){
#   if(x == 'All') ct <- sum(facil_cts)
#   else ct <- facil_cts[tolower(x)]
#   paste0(x, ' (', ct, ')')
# })
...
```{r}
survey_dates <- data.frame(survey=c('13','13','13','13-1','13-1','13-2','13-2','13-3','13-3'),
...
f1a <- ggplot(survey_dates,aes(x=start_date, y=factype, color=factor(survey))) + 
  ...
  # geom_segment(aes(x=start_date,xend=end_date,yend=survey),size=15,alpha=0.5) +
...
dat_dup_bla <- dat_dup %>% group_by(f_id,bla,survey, .drop=FALSE) %>% summarize(count=n())  %>% filter(bla != '' & grepl('13',survey))
...
p1 <- dat_dup_bla %>% filter(!f_id %in% c('O_vsnf','P_vsnf')) %>% 
  mutate(grp = paste0(f_id,bla),
         ...
         f_id = gsub('_.*','',f_id)) %>% 
  filter(bla != '' & grepl('13',survey) & factype != 'ICU') %>% 
...
tests_ndm <- sapply(unique(dat_dup_bla$f_id), function(x){
  ...
  if(nrow(dat_fid) == 4){
    # test <- prop.test(dat_fid$count, dat_fid$nsamp, correct = TRUE)$p.value
    # test <- binom.test(dat_fid$count[1]+1, dat_fid$nsamp[1]+1, p = (dat_fid$count[4]+1)/(dat_fid$nsamp[4]+1))$p.value
  ...
  c('NDM count (survey 13)' = dat_fid$count[1], 
    'Total count (survey 13)' = dat_fid$nsamp[1],
    'NDM proportion (survey 13)' = round(dat_fid$count[1]/dat_fid$nsamp[1], 2), 
    'NDM count (survey 13-3)' = dat_fid$count[4], 
    'Total count (survey 13-3)' = dat_fid$nsamp[4],
    'NDM proportion (survey 13-3)'  = round(dat_fid$count[4]/dat_fid$nsamp[4]), 
    'NDM p' = test)
})
...
tests_kpc <- sapply(unique(dat_dup_bla$f_id), function(x){
  ...
  c('KPC count (survey 13)' = dat_fid$count[1], 
    'Total count (survey 13)' = dat_fid$nsamp[1],
    'KPC proportion (survey 13)' = round(dat_fid$count[1]/dat_fid$nsamp[1],2), 
    'KPC count (survey 13-3)' = dat_fid$count[4], 
    'Total count (survey 13-3)' = dat_fid$nsamp[4],
    'KPC proportion (survey 13-3)' = round(dat_fid$count[1]/dat_fid$nsamp[4]), 
    'KPC p' = test)
})
...
prop_pvals <- cbind(t(tests_ndm), t(tests_kpc))  %>% as_tibble() %>% 
  mutate(Facility = paste(gsub('V','v',toupper(gsub('.*_','',colnames(tests_ndm)))), 
         ...
         .before = 1) %>% 
  mutate(`Total count (survey 13)` = coalesce(`Total count (survey 13)`,V9),
         `Total count (survey 13-3)` = coalesce(`Total count (survey 13-3)`,V12)) %>% 
  select(-c(V9, V12)) %>% 
  select("Facility","NDM count (survey 13)","NDM proportion (survey 13)",
         "KPC count (survey 13)","KPC proportion (survey 13)", "Total count (survey 13)",
         "NDM count (survey 13-3)","NDM proportion (survey 13-3)",
         "KPC count (survey 13-3)","KPC proportion (survey 13-3)","Total count (survey 13-3)",
         "NDM p", "KPC p") %>% arrange(`NDM p`, -`NDM proportion (survey 13)`) %>% 
  mutate("NDM p" = signif(p.adjust(`NDM p`, method = 'BH'),2),
         "KPC p" = signif(p.adjust(`KPC p`, method = 'BH'),2))

# 
# 
# prop_pvals2 <- cbind(t(tests_ndm2), t(tests_kpc2))  %>% as_tibble() %>% 
#   mutate(Facility = paste(gsub('V','v',toupper(gsub('.*_','',colnames(tests_ndm)))), 
#                             gsub('_.*','',colnames(tests_ndm))),
#          .before = 1) %>% 
#   mutate(`Total count (survey 13)` = coalesce(`Total count (survey 13)`,V9),
#          `Total count (survey 13-3)` = coalesce(`Total count (survey 13-3)`,V12)) %>% 
#   select(-c(V9, V12)) #%>% arrange(`NDM p`)
...
```{r}
# plasmid_metadat %>% filter(!is.na(ec_st)) %>% select(ec_st, bla, X1) %>% table()
# table(plasmid_metadat$bla, plasmid_metadat$ec_st)
...
f2 <- plot_dat %>% filter(!is.na(ST) & !is.na(IncF_plasmid)) %>% 
  mutate(IncF_plasmid = ifelse(IncF_plasmid==1,'IncF plasmid','No IncF plasmid'),
         ...
         grp = factor(grp, labels = c(c("Klebsiella pneumoniae"='italic("Klebsiella pneumoniae")', 'Other'='"Other"')))
         ) %>%
...
# hdat <- metadat %>% filter(bla != 'None') %>% select(gID, Region) %>% 
#   mutate(Region=ifelse(grepl('REALM',gID),'Chicago area',Region)) %>% deframe()
# hdat <- hdat[tr_nl$tip.label]  
# names(hdat) <- tr_nl$tip.label
# hdat <- c(hdat,rep(NA,Nnode(tr_nl)))
...
survey_dates_w_intervention <- data.frame(survey=factor(c('13','13','13','13-1','13-1','13-2','13-2','13-3','13-3','Intervention'),levels=c('13','13-1','13-2','13-3','Intervention')),
...
f_time <- ggplot(survey_dates_w_intervention,aes(x=start_date, y=factype, color=survey)) +
  ...
  # geom_segment(aes(x=start_date,xend=end_date,yend=survey),size=15,alpha=0.5) +
 scale_colour_discrete(guide=guide_legend(override.aes=list(size=7))) + #or legend will be too big
  labs(x='',y='',col='Survey') + scale_color_manual(values = c(cols,Intervention='darkseagreen3'), breaks = c('13','13-1','13-2','13-3')) + theme(axis.text.x=element_text(angle=0)) + theme_classic() + guides(col=guide_legend(nrow = 2)) + theme(axis.text.x = element_blank(), line = element_blank(),
...
```{r}
f4 <- plot_grid(plot_grid(f4_1, NULL, rel_widths = c(1,0.027)), 
                plot_grid(NULL, f_time, nrow = 1, rel_widths = c(2.05,10)), 
...
subtr_9_13 <- get_clusters_survey('13', 0.9)
subtr_9_301 <- get_clusters_survey('13-1', 0.9)
subtr_9_302 <- get_clusters_survey('13-2', 0.9)
subtr_9_303 <- get_clusters_survey('13-3', 0.9)
...
f5a <- all_subtr_survey %>% filter(ftype != 'icu') %>% 
  ...
  ggplot(aes(x=f_id,y=subtr_size,col=factor(survey))) + geom_jitter(size=3,alpha=0.75) +#,position = position_jitter(w = 0.2, h = 0)) +
...
snvs13 <- get_snv_dists('13')
snvs301 <- get_snv_dists('13-1')
snvs302 <- get_snv_dists('13-2')
snvs303 <- get_snv_dists('13-3')
...
fills <- rep(c('13'=adjustcolor('tan1',alpha.f = 0.5), '301'=adjustcolor('#bdc9e1',alpha.f = 0.5), '302'=adjustcolor('#67a9cf',alpha.f = 0.5), '303'=adjustcolor('#02818a',alpha.f = 0.5)),2)
...
```{r}
fs1 <- dat_dup %>% filter(bla != '' & grepl('13',survey)) %>% mutate(factype = gsub('V','v',toupper(gsub('.*_','',f_id))), f_id = gsub('_.*','',f_id), bla = factor(bla,levels = c('NDM','KPC'))) %>%
...
#   metadat_all %>% filter(bla %in% c('NDM','NDM & KPC')) %>% select(f_id) %>% group_by(f_id) %>% summarize(count=n()) %>% deframe() %>% data.frame()
...
### IncF plasmid across time and facilities. 

```{r}
bla_plas_cts <- table(plasmid_metadat %>% 
                        mutate(factype = gsub('.*_','',f_id), 
                               bla = gsub('\\+','',bla), 
                               bla = gsub('/',' & ', bla), 
                               bla = ifelse(is.na(bla), 'None',bla), 
                               factype = ifelse(factype == 'vsnf','vSNF',toupper(factype)), 
                               f_id = gsub('_.*','',f_id)) %>% 
                        filter(!is.na(f_id) & !is.na(species) & IncF_plasmid == 1) %>% 
                        select(ST, bla, survey))

table(plasmid_metadat$IncF_plasmid[plasmid_metadat$survey == 13])

bla_plas_cts
bla_plas_cts/rowSums(bla_plas_cts)
```


```{r}
fs5 <- plasmid_metadat %>% mutate(factype = gsub('.*_','',f_id), bla = gsub('\\+','',bla), bla = gsub('/',' & ', bla), bla = ifelse(is.na(bla), 'None',bla), factype = ifelse(factype == 'vsnf','vSNF',toupper(factype)), f_id = gsub('_.*','',f_id)) %>% filter(!is.na(f_id) & !is.na(species) & IncF_plasmid == 1) %>%
  ggplot(aes(x = factor(survey), y = f_id, col = bla, shape = species)) + geom_jitter(size=3) + facet_grid(factype~., scales = 'free', space = 'free') + scale_color_manual(values = bla_cols) + guides(color = guide_legend(override.aes = list(size = 3))) + scale_shape_manual(values=c(15, 19)) + labs(shape='Species',col='Carbapenemase',x='Survey',y='Facility')
fs5
```

```{r}
ggsave('figures/FigS4.pdf', fs5, width = 10, height = 5)
```
...
```{r}
bla_tips <- metadat %>% mutate(bla = gsub('\\+','',bla), bla = gsub('/',' & ', bla), #ifelse(grepl('NDM',NDM),NDM,KPC), 
...
```{r}
ggsave('figures/FigS5.pdf', p_tr, width = 7, height = 7)
```
...
tr_ndm <- mixedcarc$inputtree #drop.tip(tr, tr$tip.label[!tr$tip.label %in% as.phylo(tt)$tip.label])
yr_ndm <- unlist(sapply(tr_ndm$tip.label, function(x) metadat_all$survey_year[metadat_all$gID == x]))
# convert edge lengths to substitutions per genome (rather than per site)
#glength <- 5301922
#tr_ndm$edge.length <- tr_ndm$edge.length*glength
...
#tr_st147$edge.length <- tr_st147$edge.length*glength
pdf('figures/FigS6.pdf')
# par(mfrow = c(1,2))
rtt_ndm <- roottotip(tr_ndm, yr_ndm, showTree = FALSE)
# rtt_st147 <- roottotip(tr_st147, yr_st147, showTree = FALSE)
dev.off()
```
...
```{r}
fs9 <- plot_grid(plot_grid(subtr_9_13$tr_survey + theme(legend.position = 'none') + labs(title = 'Survey 13') + geom_treescale(y = 1) + theme(plot.title = element_text(hjust = 0.5)),
                    subtr_9_301$tr_survey + theme(legend.position = 'none') + labs(title = 'Survey 13-1') + geom_treescale(x = 0.000008, y = 5) + theme(plot.title = element_text(hjust = 0.5)),
                    subtr_9_302$tr_survey + theme(legend.position = 'none') + labs(title = 'Survey 13-2') + geom_treescale(x = 0.000001, y = 12) + theme(plot.title = element_text(hjust = 0.5)),
                    subtr_9_303$tr_survey + theme(legend.position = 'none') + labs(title = 'Survey 13-3') + geom_treescale(x = 0.000015, y = 10) + theme(plot.title = element_text(hjust = 0.5))),
          get_legend(f4_1))
fs9
```

```{r}
ggsave('figures/FigS7.pdf', fs9, width = 7, height = 7)
```

### LOS of different facilities.

```{r}
s10a <- dat %>% mutate(survey = ifelse(survey == '301', '13-1', survey),
         survey = ifelse(survey == '302', '13-2', survey),
         survey = ifelse(survey == '303', '13-3', survey),
         survey = as.factor(survey),
         f_id = gsub('_.*','',f_id),
         factype = gsub('V','v',toupper(factype))) %>% 
  ggplot(aes(x = factype, y = as.numeric(facility_day))) + geom_violin() + scale_y_log10() +
  labs(x = '', y = 'Length of stay')

s10b <- dat %>% mutate(survey = ifelse(survey == '301', '13-1', survey),
         survey = ifelse(survey == '302', '13-2', survey),
         survey = ifelse(survey == '303', '13-3', survey),
         survey = as.factor(survey),
         f_id = gsub('_.*','',f_id)) %>% 
  filter(factype == 'vsnf') %>% 
  ggplot(aes(x = f_id, y = as.numeric(facility_day))) + geom_boxplot() +
  labs(x = 'vSNF', y = 'Length of stay') #+ scale_y_log10()

dat %>% mutate(survey = ifelse(survey == '301', '13-1', survey),
         survey = ifelse(survey == '302', '13-2', survey),
         survey = ifelse(survey == '303', '13-3', survey),
         survey = as.factor(survey),
         f_id = gsub('_.*','',f_id)) %>%
  filter(factype == 'vsnf') %>% 
  group_by(f_id) %>% summarize(median_los=median(as.numeric(facility_day), na.rm = T)) %>% 
    dplyr::select(f_id, median_los) %>% deframe()

s10 <- plot_grid(s10a, s10b)
s10
```

```{r}
# ggsave('figures/FigS10.pdf', s10, width = 10, height = 6)
```
...
# all_snvs$loc1 <- gsub('_.*','',all_snvs$loc1)
# all_snvs$loc2 <- gsub('_.*','',all_snvs$loc2)

fs11_1 <- all_snvs %>% ggplot(aes(x=Freq, fill=intra)) + geom_histogram(position="identity",bins=30) + facet_grid(ftypes~survey) + labs(x='Pairwise SNV distance', y='\nCount',fill='') + scale_fill_manual(values = c('Inter-facility pair'=adjustcolor('darkslategray3',alpha.f=0.5),'Intra-facility pair'=adjustcolor('lightsalmon',alpha.f=0.5))) 
...
fs11 <- plot_panel_cols(fs11_1, fills)
fs11 
```

```{r}
ggsave('figures/FigS8.pdf', fs11, width = 11, height = 7)
```
...
sf <- snvs_sub %>% #mutate(lt20 = ifelse(lt20 == '≤12', '<=12',lt20)) %>% 
...
fills <- c('13'=adjustcolor('tan1',alpha.f = 0.5), '301'=adjustcolor('#bdc9e1',alpha.f = 0.5), '302'=adjustcolor('#67a9cf',alpha.f = 0.5), '303'=adjustcolor('#02818a',alpha.f = 0.5))
snvs_flow <- plot_panel_cols(sf, fills)
snvs_flow 
```

```{r}
ggsave('figures/FigS9.pdf', snvs_flow, width = 9, height = 7)
```
